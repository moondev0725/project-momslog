...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 30개 게시글 생성
        self.stdout.write("공감 게시글 생성 시작...")
        created_count = 0
        ...
        for i in range(30):
            try:
                ...
                self.stdout.write(f"✗ 게시글 {i+1} 생성 실패: {e}")

        total = FreePost.objects.filter(category='육아 수다').count()
        self.stdout.write(
            self.style.SUCCESS(f"\n✓ 총 {created_count}개 게시글 생성 완료! (전체: {total}개)")
        )
